...
#else
#    rm alice.logs
#    nohup nucypher alice run --dev --federated-only --teacher-uri 52.14.207.225:9151 > alice.logs &
#    sleep 5
#    avk=$(cat alice.logs | grep "Alice Verifying Key" | awk -F "\ " '{print $4}')
#fi
if [[ $bobAlive ]]; then
    ...
    rm bob.logs
    nohup nucypher bob run --dev --federated-only --teacher-uri 127.0.0.1:10151 > bob.logs &
    sleep 5
    bob=$(cat bob.logs | grep "Key" | awk -F "\ " '{print $4}' | tr "\n" ","| sed 's/.$//')
#    echo "hola"
fi
